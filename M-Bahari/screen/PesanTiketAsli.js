...
fetch('http://expressbahari.com/php_mobile/Cek_Promo.php', {
  ...
  cekKeberangkatan=()=>{
    ...
    if(this.state.jadwalPergi==true && this.state.jadwalPulang==false){
      this.setState ({tampilDetailHarga:false  }); 
      ToastAndroid.show('1Jadwal Pulang Tidak Tersedia, Silahkan Periksa Kembali Jadwal Anda', ToastAndroid.LONG);
    }
    ...
    if(this.state.JenisTiket=='Sekali Jalan'){
      ...
      this.setState ({tampilDetailHarga:false  });
     ToastAndroid.show('2Jadwal Tidak Tersedia, Silahkan Periksa Kembali Jadwal Anda', ToastAndroid.LONG);
    }
   }
  }
  }
              ...
              <View style={{flexDirection:'row', marginTop:15, alignItems:'center'}}>
                <View style={{width:'40%', alignItems:'center'}}>
                  ...
                  {this.state.viewvisiblePP?<Text style={{fontSize: 15, color:'#000', marginTop:9, fontWeight: 'bold', position:'absolute'}}>{moment(this.state.date2).format('DD MMM YYYY')}</Text>:<Icon name='add' style={{fontSize:30,marginTop:4,position:'absolute'}} />}
                   ...
                   onDateChange={(date) => {this.setState({date2: date, JenisTiket:'PP',tampilJadwal2:false,tampilDetailHarga:false,tampilJadwal:false,tampilClose:true}),this.loadingsJeda(),this.toggleSwitchPP()}}
                  />
                  <View>
                    {
    ...
    <View style={{borderTopLeftRadius:10,borderTopRightRadius:10, backgroundColor:'#fff', marginTop:5, width:'95%'}}>
        ...
        {this.state.tampilJadwal2?
            ...
            {this.state.lengthJadwal.map((lengthJadwal)=>{
                return (
                <View key={lengthJadwal.id} style={{width:'100%'}}>
                    ...
                    <CardItem style={{height:30, backgroundColor:'transparent',marginTop:-10}}>
                         <Text style={{fontSize: 13, fontFamily:'SourceSansProBold'}}>Berangkat - {moment(this.state.date).format('DD MMM YYYY')}, {this.state.ETD} - {this.state.ETA}</Text>
                         <Right>
          ...
          }
          {
            ...
            <View style={{width:'100%', alignItems:'center'}}>   
              {this.state.arrayPP.map((arrayPP)=>{
                return (
                    <View key={arrayPP.id} style={{width:'100%'}}>
                    ...
                    <CardItem style={{height:30, backgroundColor:'transparent'}}>
                         <Text style={{fontSize: 13, fontFamily:'SourceSansProBold'}}>Pulang - {moment(this.state.date2).format('DD MMM YYYY')}, {this.state.arrayPP[0].berangkat} - {this.state.arrayPP[0].tiba}</Text>
                         <Right>
