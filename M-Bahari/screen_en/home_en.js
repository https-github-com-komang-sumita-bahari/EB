 ...
 const { navigation } = this.props;
  ...
  toggleTab1() {
    ...
    this.setState({
     ...
     this.props.navigation.navigate('Pesan_Tiket_en');
  }
  toggleTab2() {
    this.setState({
      tab1: false,
    ...
    this.props.navigation.navigate('Pesanan_en',{jmlhNotif:this.state.jmlhNotif});
    this.setState({
      tab1:true,
      tab2:false,
    });
...
convertTextToUpperCase = () => {
 ...
 async componentWillMount() {
    ...
    StatusBar.setHidden(false);
     ...
     db.transaction(tx => {
      tx.executeSql('select * from tutorial;',[],(_, { rows: { _array } }) => this.setState({ Tutorial: _array })
      );
      tx.executeSql('select * from login_session;',[],(_, { rows: { _array } }) => this.setState({ komang: _array.id_user })
      );
      tx.executeSql('select * from tbl_settings;',[],(_, { rows: { _array } }) => this.setState({ Setting: _array[0].s_bahasa })
      );
    });

    setTimeout(()=>{
      if (this.state.Tutorial.length==0){
...
loadings=()=>{
    ...
    setTimeout(()=>{
      this.props.navigation.navigate('Detail_Promo_en',{idpromo:this.state.idpromo,kodepromo:this.state.kodepromo,deskripsi_en:this.state.deskripsi_en,namapromo:this.state.namapromo_en,berlaku:this.state.berlaku,image:this.state.image,idkupon:this.state.idkupon,deskripsi:this.state.deskripsi,namakupon:this.state.namakupon,point:this.state.point,imageKupon:this.state.imageKupon});
      this.setState({loading1: !this.state.loading1});
      }, 100);
  ...
  loadingsKupon=()=>{
    ...
    setTimeout(()=>{
      ...
      this.props.navigation.navigate('Detail_kupon_en',{idkupon:this.state.idkupon,deskripsi_en:this.state.deskripsi_en,namakupon_en:this.state.namakupon_en,point:this.state.point,rpoint:this.state.rpoint,imageKupon:this.state.imageKupon});
      this.setState({loading3: !this.state.loading3});
...
getPromo=(id_promo,kode_promo,deskripsi,deskripsi_en,nama,nama_en,berlaku,image,image_en)=>
    {
        this.setState({idpromo:id_promo})
        this.setState({kodepromo:kode_promo})
        this.setState({deskripsi:deskripsi})
        this.setState({deskripsi_en:deskripsi_en})
        this.setState({namapromo:nama})
        this.setState({namapromo_en:nama_en})
        this.setState({berlaku:berlaku})
        this.setState({image:image})
        this.loadings();
    }
    getKupon=(id_kupon,nama,deskripsi,nama_en,deskripsi_en,point,image,image_en)=>
    {
        this.setState({idkupon:id_kupon})
        this.setState({namakupon:nama})
        this.setState({deskripsi:deskripsi})
        this.setState({namakupon_en:nama_en})
        this.setState({deskripsi_en:deskripsi_en})
 ...
 semuaPromo=()=>{
  this.props.navigation.navigate('semua_promo_en');
 }
 DetailETIket=()=>{
  this.props.navigation.navigate('detail_eTiket');
 }
 semuaKupon=()=>{
  this.props.navigation.navigate('semua_kuponcabang_en');
 }    
 point =()=>{
        ...
        <Content>
         <ScrollView>
                ...
                <View style={{flexDirection: 'row', marginTop:10, marginBottom:10}}>
                    <TouchableOpacity onPress={() =>this.saveData()}>
                      <View style={{alignItems:'center'}}>
                          ...
                          <Text style={{fontSize:10, color:'#ffffff', marginTop:4, fontFamily:'SourceSansProBold'}}>Book Ticket</Text>
                      </View>
                    </TouchableOpacity>
                    <TouchableOpacity  onPress={() => this.props.navigation.navigate('TampilJadwal_en')}>
                      <View style={{alignItems:'center', marginLeft:10,}}>
                          <Image source={require('../images/jadwal.png')} style={{width:22, height:23}}/>
                          <Text style={{fontSize:10, color:'#ffffff', marginTop:6,fontFamily:'SourceSansProBold'}}>Schedule</Text>
                      </View>
                    </TouchableOpacity>
          ...
          (
            ...
            <View style={{ flexDirection: 'row', width: '100%'}}>
              <Text style={{fontSize: 22, marginTop:10, fontFamily:'SourceSansProBold'}}>   Current Promo</Text>
              <Right style={{marginRight:10}}>
              <TouchableOpacity onPress={this.semuaPromo}>
                <Text style={{fontSize: 14, marginTop:10, color:'#3F81B5',fontFamily:'SourceSansProBold'}}>See All  <Icon active name="ios-arrow-forward" style={{fontSize: 14,color:'#3F81B5'}} /></Text>
              </TouchableOpacity>
              </Right>
            </View>
          ...
          (
            <View key = { key } style = { styles.promo }>
                <TouchableOpacity onPress={this.getPromo.bind(this, promo.id_promo,promo.kode_promo,promo.deskripsi,promo.deskripsi_en,promo.nama,promo.nama_en,promo.berlaku,promo.image)}>
                  ...
                  <Text numberOfLines={1} style={{fontSize:18, marginLeft:10,width:300, fontFamily:'SourceSansProBold'}}>{promo.nama_en}</Text>
                  <Text numberOfLines={2} style={{fontSize:12, marginLeft:11,width:300, fontFamily:'Gadugi'}}>{promo.deskripsi_en}</Text>
                </TouchableOpacity>  
            ...
            <View style={{ flexDirection: 'row', width: '100%', marginTop:25 }}>
              <Text style={{fontSize: 22, fontFamily:'SourceSansProBold', marginTop:10}}>   Redeem Point's</Text>
              <Right style={{marginRight:10}}>
          ...
          (
            <View key = { key } style = { styles.kupon }>
                <TouchableOpacity onPress={this.getKupon.bind(this, kupon.id_kupon,kupon.nama,kupon.nama_en,kupon.deskripsi,kupon.deskripsi_en,kupon.point,kupon.image)}>
                  ...
                  <CardItem style={{height:20}}>
                      <Text style={{fontFamily:'SourceSansProBold', fontSize:14}}>{kupon.nama_en}</Text>
                    </CardItem>
                    </View>
                    <CardItem style={{height:10, backgroundColor:'transparent'}}>
                    <Image source={require('../images/icon_point.png')} style={{width:18, height:14}}/>
                      <Text style={{marginLeft:3, fontSize:16, color:'#ff6600',fontFamily:'SourceSansProBold',}}>{kupon.rpoint} Points</Text>
                    </CardItem>
