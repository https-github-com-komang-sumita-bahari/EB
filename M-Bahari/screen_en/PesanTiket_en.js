...
fetch('http://expressbahari.com/php_mobile/Cek_Promo.php', {
  ...
  body: JSON.stringify({
    ...
    jumlah_penumpang :(parseInt(this.state.ValueDewasa)+parseInt(this.state.ValueAnak)),
  })
      ...
      this.setState({
        ...
        ToastAndroid.show('Promo Code Not Applicable, Please Use Other Promo Codes', ToastAndroid.LONG);
...
cekJadwal=()=> {
  ...
  body: JSON.stringify({
   ...
   ToastAndroid.show('Schedule not available', ToastAndroid.LONG); 
    if (this.state.KelasTiket==''){
      this.setState({
        viewvisiblePenumpang:false,
        colorbg:'#fcfbf8',
...
cekJadwalHarga=()=> {
  ...
  body: JSON.stringify({
    ...
    .catch((error) => {
   this.setState({loading2:false});
   ToastAndroid.show('Departure Schedule not available', ToastAndroid.LONG); 
...
cekPP=()=> {
  ...
  body: JSON.stringify({
   ...
   this.clearPP();
   this.setState({loading2:false});   
   ToastAndroid.show('Return Schedule is not available', ToastAndroid.LONG); 
...
clearPP=()=>{
  ...
  if(this.state.JenisTiket=='PP'){
   ...
   if(moment(this.state.date2+' '+this.state.arrayPP [0].berangkat).format("DD-MM-YYYY HH:mm")<moment(this.state.date+' '+this.state.lengthJadwal   [0].berangkat).format("DD-MM-YYYY HH:mm")){
    ToastAndroid.show('Please Change the Return Date', ToastAndroid.LONG); 
   }
   else{
     this.saveData();
   }
  ...
  toggleSwitchPP = () => {
    this.setState({viewvisiblePP: true, show_ringkasan:false,JenisTiket:'PP'});
    if(this.state.KelasTiket==''){
      this.cekPPTanggal()
    }
...
cekChange=()=>{
    if(this.state.koderute==''  ){
      ToastAndroid.show('Please Select a Departure Route', ToastAndroid.LONG);
    }
    else{
      this.change()
    }
  ...
  change= ()=>{
    this.setState({jadwalPergi:false, jadwalPulang:false, show_ringkasan:false,ButtonStateHolder:true,bgcolor:'#fcfbf8'})
...
_toggleModal = () =>{
  ...
  _modalDewasa=()=>{
    this.setState({ ModalDewasa: !this.state.ModalDewasa, judulheaderAccordion:'Adult'});
    this.setState({ footerhide: !this.state.footerhide })
    if(this.state.ValueDewasa==0){
        ...
        if(this.state.cekTotalPenumpang-this.state.ValueInfant==0){
          this.setState({show_ringkasan:false, viewvisibleKelas:false,colorVIP: '#F4F0E5', colorEXE:'#F4F0E5',colorTxtVip:'#000', colorTxtEXE:'#000',ButtonStateHolder:true,bgcolor:'#fcfbf8',KelasTiket:''});
        }
        setTimeout(()=>{
          this.ReHitung();
  ...
  _modalAnak=()=>{
    this.setState({ Modalanak: !this.state.Modalanak, judulheaderAccordion:'Child'});
    this.setState({ footerhide: !this.state.footerhide })
    if(this.state.ValueAnak==0){
        ...
        if(this.state.cekTotalPenumpang==0){
          this.setState({show_ringkasan:false, viewvisibleKelas:false,colorVIP: '#F4F0E5', colorEXE:'#F4F0E5',colorTxtVip:'#000', colorTxtEXE:'#000',ButtonStateHolder:true,bgcolor:'#fcfbf8',KelasTiket:''});
        }
        setTimeout(()=>{
          this.ReHitung();
  ...
  _modalInfant=()=>{
    this.setState({ ModalInfant: !this.state.ModalInfant, judulheaderAccordion:'Infant'});
    this.setState({ footerhide: !this.state.footerhide })
  ...
  loadingsKuponPromo=()=>{
    ...
    setTimeout(()=>{
        ...
        this.setState({totalDiskon:this.state.dataPromo[0].total_nominal});
        if(this.state.JenisTiket=='PP'){
...
loadings3=()=>{
  ...
  Navigate_To_Second_Activity=(rute,kode_asal,kode_tujuan,asal,tujuan,kode_rute, asuransi,pass_pelabuhan,infant,kode_rute2,pass_pelabuhan2,infant2,potongan_pp,nama_pelabuhan,nama_pelabuhan2,rute2,asal2,kode_asal2,tujuan2,kode_tujuan2)=>
    {
      ...
      this.setState({ data1:[],data2:[],jadwalPergi:false, show_ringkasan:false,ButtonStateHolder:true,bgcolor:'#fcfbf8'})
  ...
  getCekStok() {
    ...
    return fetch('http://expressbahari.com/php_mobile/cek_stoktiket.php', {
    method: 'POST',
    headers: {
      'Accept': 'application/json',
      'Content-Type': 'application/json',
    ...
    body: JSON.stringify({
      ...
      kapasitas : this.state.ValueKapasitas,
	    jumlah_penumpang :(parseInt(this.state.ValueDewasa)+parseInt(this.state.ValueAnak)),
      jadwal_id : this.state.idKeberangkatan,
      kelas : this.state.KelasTiket,
      jenis_tiket:this.state.JenisTiket,
      kapasitas_pp : this.state.ValueKapasitasPP,
      jadwal_id_pp : this.state.idPulang,
          ...
          .then((responseJson) => {
            ...
            this.setState({
              ...
              if(responseJson[0].status==1){
                if(this.state.JenisTiket=='PP'){
                  if(responseJson[0].status_pp==1){
                    this.loadingsPesan();
                  }
                  else{
                    this.setState({loading3: !this.state.loading3});
                    Alert.alert('Return ticket has expired')
                  }
                }
                else{
                  this.loadingsPesan(); 
                ...
                this.setState({loading3: !this.state.loading3});
                Alert.alert('Departure ticket has expired')
              }
            });
  ...
  body: JSON.stringify({
    ...
    id_kupon:this.state.idkupon,
      ...
      this.setState({
        ...
        ToastAndroid.show('Coupons Not Applicable, Please Use Other Coupons', ToastAndroid.LONG);
       //this._toggleModalInputPromo();
      });
...
getKupon=(id_kupon_saya,id_kupon,id_user,status)=>
 ...
 saveData = () =>{
        ...
        let AsyncPotongan = this.state.dataPromo[0].total_nominal;
    ...
    _renderHeader = section => {
      return (
        <Card style={{height:40,borderRadius:8, }}>
          ...
          <Right>
            {
              this.state.show_icon?<Text style={{fontSize: 13, color:'#D3D3D3'}}>Fill in the data</Text>:
  ...
  render() {
      ...
      <Dialog
        ...
        <Header style={{backgroundColor:'#3F81B5',}}>
         ...
         <View style={styles.view}>
          <Title style={{marginTop:15,fontFamily:'SourceSansProBold', fontSize:21}}>Find My Schedule</Title>
          <Right>
       ...
       <BottomSheet
          ...
          onBackdropPress={this._toggleModal}>
            ...
            <View style={styles.view}>
              <Title style={{marginTop:15, fontFamily:'SourceSansProBold'}}>Choose Rute</Title>
              <Right>
    ...
    <View style={{width:280,backgroundColor:'#fff', borderRadius:10, alignItems:'center'}}>
      <CardItem style={{backgroundColor:'transparent'}}>
          <Text>Please Pick Your Departure Date</Text>
      </CardItem>
    ...
    <View style={{width:280,backgroundColor:'#fff', borderRadius:10, alignItems:'center'}}>
      <CardItem style={{backgroundColor:'transparent'}}>
          <Text>Please Pick Your Return Date</Text>
      </CardItem>
      <View style={{backgroundColor:'#D3D3D3', width:'100%', height:1}}/>
      <View style={{marginTop:6, marginBottom:20}}> 
            ...
            <View style={{alignItems:'center', backgroundColor:'#fff'}}>  
                ...
                <TouchableOpacity onPress={this._toggleModal}>
                    <View style={{height:40, backgroundColor:'#F4F0E5', width:'97%', borderRadius:10,}}>
                      <View style={{flexDirection:'row', width:'100%',height:35, alignContent:'center'}}>
                          <Text style={{fontSize:13, marginLeft:10, marginTop:9,fontFamily:'Roboto'}}>From    </Text>
                  ...
                  <View style={{height:40, backgroundColor:'#F4F0E5', width:'97%', borderRadius:10,marginTop:10}}>
                    <View style={{flexDirection:'row', width:'100%',height:35, alignContent:'center'}}>
                        <Text style={{fontSize:13, marginLeft:10, marginTop:9}}>To         </Text>
                        <View style={{height:30, width:1, backgroundColor:'#606060',marginTop:5}}></View>
              ...
              <View style={{flexDirection:'row', marginTop:15, alignItems:'center'}}>
                <View style={{width:'40%', alignItems:'center'}}>
                  {this.state.viewvisiblePergi?<Text style={{fontSize:12}}>Departure Date</Text>:<Text style={{fontSize:12, color:'#F4F0E5'}}>Departure Date</Text>}
                  ...
                  {this.state.viewvisible?<Text style={{fontSize:12, marginTop:18}}>Return Date</Text>:<Text style={{fontSize:12, marginTop:18, color:'#F4F0E5'}}>Return Date</Text>}
                  ...
                  {this.state.viewvisiblePP?<Text style={{fontSize: 15, color:'#000', marginTop:9, fontWeight: 'bold', position:'absolute'}}>{moment(this.state.date2).format('DD MMM YYYY')}</Text>:<Icon name='add' style={{fontSize:30,marginTop:4,position:'absolute'}} />}
                   ...
                   onDateChange={(date) => {this.setState({date2: date, JenisTiket:'PP',tampilJadwal2:false,tampilDetailHarga:false,tampilJadwal:false,tampilClose:true,jadwalPergi:true,jadwalPulang:false,ButtonStateHolder:true,bgcolor:'#fcfbf8'}),this.toggleSwitchPP()}}
                  />
                  <View>
                    {
                ...
                <View style={{width:'55%'}}>
                {this.state.viewvisiblePenumpang?<Text style={{fontSize:12, marginLeft:10}}>Passangers</Text>:<Text style={{fontSize:12, marginLeft:10,color:'#F4F0E5'}}>Passangers</Text>}
               <View style={{height:122, backgroundColor:this.state.colorbg, width:'93%', borderRadius:10,marginTop:5, marginLeft:10, alignItems:'center'}}>
                  <View style={{flexDirection:'row', margin:8, alignItems:'center'}}>
                    {this.state.viewvisiblePenumpang?<View style={{width:'45%'}}>
                     ...
                     <Text style={{fontSize:11,}}>> 12 Years Old</Text>
                    </View>:<View style={{width:'45%'}}>
                     <Text style={{fontSize:12,color:'#F4F0E5'}}>Adult</Text>
                     <Text style={{fontSize:11,color:'#F4F0E5'}}>> 12 Years Old</Text>
                    </View>
                  }
                ...
                <Button block style={{backgroundColor:'#3F81B5', height:30,borderRadius:5 }} onPress={this._modalDewasa}>{ this.state.tomboldewasa?<Text>{this.state.ValueDewasa}</Text>:<Text><Icon name='add' style={{fontSize:18,marginTop:3,position:'absolute', color:'#fff'}}></Icon></Text>}</Button>
                  ...
                  <BottomSheet
                    ...
                    onBackdropPress={this._modalDewasa}>
                      ...
                      <Button transparent style={{marginLeft:-20}} onPress={this._modalDewasa}>
                        ...
                        <Title style={{marginTop:15}}>Adult<Text ></Text>{this.state.title}</Title>
                    ...
                    { this.state.tomboldewasa?
                      <View style={{alignItems:'center', marginTop:15, width:'100%'}}>
                        <View style={{width:'95%'}}>
                          <Button block onPress={this.simpanModalDewasa} style={{borderRadius:10, backgroundColor:'#3F81B5'}}><Text style={{color:'#ffffff', alignItems:'center'}}>Save</Text></Button>
                        </View>
                      </View>:
                  ...
                  <View style={{flexDirection:'row', margin:8, marginTop:-2, alignItems:'center'}}>
                    {this.state.viewvisiblePenumpang?<View style={{width:'45%'}}>
                     ...
                     <Text style={{fontSize:11,}}>2-12 Years Old</Text>
                    </View>:<View style={{width:'45%'}}>
                     <Text style={{fontSize:12,color:'#F4F0E5'}}>Child</Text>
                     <Text style={{fontSize:11,color:'#F4F0E5'}}>2-12 Years Old</Text>
                    </View>
                    }
                    <BottomSheet
                    ...
                    onBackdropPress={this._modalAnak}>
                      ...
                      <Button transparent style={{marginLeft:-20}} onPress={this._modalAnak}>
                        ...
                        <Title style={{marginTop:15}}>Child <Text ></Text>{this.state.title}</Title>
                        <Right style={{alignItems:'center'}}>
                    ...
                    { this.state.tombolanak?
                      <View style={{alignItems:'center', marginTop:15, width:'100%'}}>
                        <View style={{width:'95%'}}>
                          <Button block onPress={this.simpanModalAnak} style={{borderRadius:10, backgroundColor:'#3F81B5'}}><Text style={{color:'#ffffff', alignItems:'center'}}>Save</Text></Button>
                        </View>
                      </View>:
                        <View style={{alignItems:'center', marginTop:30}}>
                          <Image source={require('../images/icon_penumpangdewasa.png')} style={{width:270, height:205}}/>
                ...
                <Button block style={{backgroundColor:'#3F81B5', height:30,borderRadius:5 }} onPress={this._modalAnak}>{ this.state.tombolanak?<Text>{this.state.ValueAnak}</Text>:<Text><Icon name='add' style={{fontSize:18,marginTop:3,position:'absolute', color:'#fff'}}></Icon></Text>}</Button>
                  ...
                  <View style={{flexDirection:'row', margin:8, marginTop:-2,alignItems:'center'}}>
                    {this.state.viewvisiblePenumpang?<View style={{width:'45%'}}>
                     <Text style={{fontSize:12,}}>Infant</Text>
                     <Text style={{fontSize:11,}}>0-2 Years Old</Text>
                    </View>:<View style={{width:'45%'}}>
                     <Text style={{fontSize:12,color:'#F4F0E5'}}>Infant</Text>
                     <Text style={{fontSize:11,color:'#F4F0E5'}}>0-2 Years Old</Text>
                    </View>
                  }
                ...
                <BottomSheet
                    ...
                    onBackdropPress={this._modalInfant}>
                      ...
                      <Button transparent style={{marginLeft:-20}} onPress={this._modalInfant}>
                        ...
                        <Title style={{marginTop:15}}>Infant <Text ></Text>{this.state.title}</Title>
                        <Right style={{alignItems:'center'}}>
                    ...
                    { this.state.tombolinfant?
                      <View style={{alignItems:'center', marginTop:15, width:'100%'}}>
                        <View style={{width:'95%'}}>
                          <Button block onPress={this.simpanModalInfant} style={{borderRadius:10, backgroundColor:'#3F81B5'}}><Text style={{color:'#ffffff', alignItems:'center'}}>Save</Text></Button>
                        </View>
                      </View>:
                        <View style={{alignItems:'center', marginTop:30}}>
                          <Image source={require('../images/icon_penumpangdewasa.png')} style={{width:270, height:205}}/>
               ...
               <View style={{width:'92%', marginBottom:20}}>
                  {this.state.viewvisibleKelas?<Text style={{fontSize:12, marginTop:18}}>Seat Class</Text>:<Text style={{fontSize:12, marginTop:18,color:'#F4F0E5'}}>Seat Class</Text>
                  }
                  ...
                  <View style={{flexDirection:'row', marginTop:5}}>
                    ...
                    <Button transparent style={{height:35,borderRadius:5, width:'100%', alignItems:'center',backgroundColor:'#fcfbf8'}} ><Text style={{color:'#F4F0E5'}}>EXECUTIVE</Text></Button>
                    </View>
                    }
...
{ this.state.show_ringkasan?
  ...
  <View style={{width:'100%', alignItems:'center'}}>
    <Text style={{fontSize: 18, marginTop:15, fontFamily:'SourceSansProBold',marginBottom:10}}>YOUR BOOKING SUMMARY</Text>
            ...
            {this.state.tampilDetailJadwal1.map((tampilDetailJadwal1)=>{
                ...
                <View key={tampilDetailJadwal1.id} style={{width:'100%'}}>
                    ...
                    <CardItem style={{height:30, backgroundColor:'transparent',marginTop:-10}}>
                         <Text style={{fontSize: 13, fontFamily:'SourceSansProBold'}}>Departure - {moment(this.state.date).format('DD MMM YYYY')}, {tampilDetailJadwal1.berangkat} -{tampilDetailJadwal1.tiba}</Text>
                         <Right>
                        {this.state.tampilUbahJadwal?<TouchableOpacity onPress={this.UbahJadwal}><Text style={{fontSize: 8,color:'#3F81B5',fontWeight: 'bold', marginTop:-2}}>  <Icon active name="ios-create" style={{fontSize: 18, color:'#3F81B5'}} /> Change Time</Text>
                        </TouchableOpacity>
                        :null
                        }
                        </Right>
            ...
            <View style={{width:'100%', alignItems:'center'}}>   
              {this.state.tampilDetailJadwal2.map((tampilDetailJadwal2)=>{
                return (
                    ...
                    <CardItem style={{height:30, backgroundColor:'transparent',marginTop:-10}}>
                         <Text style={{fontSize: 13, fontFamily:'SourceSansProBold'}}>Departure - {moment(this.state.date).format('DD MMM YYYY')},{this.state.tampilDetailJadwal1[0].berangkat} - {this.state.tampilDetailJadwal1[0].tiba}</Text>
                         <Right>
                        {this.state.tampilUbahJadwal?<TouchableOpacity onPress={this.UbahJadwal}><Text style={{fontSize: 8,color:'#3F81B5',fontWeight: 'bold', marginTop:-2}}>  <Icon active name="ios-create" style={{fontSize: 18, color:'#3F81B5'}} /> Change Time</Text>
                        </TouchableOpacity>
                        :null
                    ...
                    <CardItem style={{height:30, backgroundColor:'transparent'}}>
                         ...
                         <Right>
                        {this.state.tampilUbahJadwal?<TouchableOpacity onPress={this.UbahJadwal2}><Text style={{fontSize: 10,color:'#3F81B5',fontWeight: 'bold', marginTop:-2}}> <Icon active name="ios-create" style={{fontSize: 18, color:'#3F81B5'}} /> Change Time</Text>
                        </TouchableOpacity>
                        :null
                        }
                        </Right>
                ...
                <CardItem style={{backgroundColor:'transparent'}}>
                  ...
                  <View style={{width:'80%'}}>
                    ...
                    <CardItem style={{height:5,}}>
                        ...
                        <Text style={{fontSize: 11, color:'#009900', marginLeft:-12}}>Refund Available</Text>
                      
                        <Text style={{fontSize: 11,}}>      </Text>
                        
                        <Icon active name="md-checkmark-circle-outline" style={{fontSize: 18, color:'#009900'}}/>
                        <Text style={{fontSize: 11, color:'#009900', marginLeft:-12}}>Reschedule Available</Text>
                    </CardItem>
    ...
    { 
      this.state.hideTombolGunakan?
        <TouchableOpacity onPress={this._toggleModalInputPromo}>
          <Card style={{borderRadius:10, height:40}}>
            <CardItem style={{backgroundColor:'transparent', marginTop:-2}}>
              ...
              <Text style={{fontSize: 12, color:'#3F81B5', fontWeight: 'bold',}}>Use Coupon or Promo Code</Text>
            </CardItem>
          </Card>
          </TouchableOpacity>
        ...
        <TouchableOpacity onPress={this.batalKupon  }>
          ...
          <Card style={{borderRadius:10, height:40, marginBottom:20, marginTop:10}}>
            <CardItem style={{backgroundColor:'transparent', marginTop:-2}}>
              <Icon active name="close" style={{fontSize: 20, color:'#3F81B5'}}/>
              <Text style={{fontSize: 12, color:'#3F81B5', fontWeight: 'bold',}}>Cancel Using Coupon/Promo</Text>
            </CardItem>
          </Card>
        </TouchableOpacity>
      </CardItem>
      ...
      {this.state.HideTotalBayarDiskon?<CardItem style={{backgroundColor:'transparent'}}>
        ...
        <Right>
          <Text style={{fontSize: 16,fontWeight: 'bold'}}>Rp. {this.state.totalHargaPromo   },-</Text>
        </Right>
      </CardItem>:<CardItem style={{backgroundColor:'transparent'}}>
        <Text style={{fontSize: 16,fontWeight: 'bold'}}>Total price     </Text>
        <Right>
          <Text style={{fontSize: 16,fontWeight: 'bold'}}>Rp. {this.state.rpTotalHarga  },-</Text>
        </Right>
      </CardItem>
      }
      </View>
...
<BottomSheet
         ...
         <Left>
          ...
          <Title style={{marginTop:15,}}>Use Coupon/Promo</Title>
          </View>
        </Header>
          ...
          <ListItem>
                ...
                <Card>
                  <View style={{flexDirection: 'row',width:'100%', alignSelf: 'center',}}>
                    <View  style={{width:'60%', alignItems:'center',marginLeft:15}}>
                      <Reinput label='Input Promo Code' labelActiveColor='#3F81B5' underlineActiveColor='#3F81B5' fontSize={18} value={this.state.kodepromo} onChangeText={text => this.setState({ kodepromo  : text })}/>
                    </View>
                    
                    <View  style={{width:'40%', alignSelf:'center'}}>
                      <Button style={{marginLeft:10, backgroundColor:'#3F81B5', height:45}} onPress={() =>
                  this.CekPromo()}>
                        <Text style={{fontSize:12}}>Use</Text>
                      </Button>
                    </View>
                  </View>
                </Card>
            ...
            <View>
              <Text style={{fontSize:20, marginTop:10, marginBottom:5,marginLeft:15, fontFamily:'SourceSansProBold'}}>My Coupon</Text>
            <ScrollView>
            {
        ...
        <Footer style={{backgroundColor:this.state.bgcolor}}>
          ...
          onPress={this.Periksa} >
            <Text style={{color:'#ffffff', fontFamily:'SourceSansProBold', fontSize:18, alignItems:'center'}}>Purchase Ticket</Text>
        </TouchableOpacity>
        </Footer>
      </Container>
    );
