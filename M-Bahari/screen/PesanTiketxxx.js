...
ReHitung=()=>{
  ...
  this.setState({rpTotalHargaPP:(parseInt(this.state.rpHargaDewasa) * parseInt(this.state.ValueDewasa)) + (parseInt(this.state.rpHargaAnak) * parseInt(this.state.ValueAnak)) + (parseInt(this.state.hargaInfant) * parseInt(this.state.ValueInfant))+(parseInt(this.state.rpHargaDewasaPP) * parseInt(this.state.ValueDewasa)) + (parseInt(this.state.rpHargaAnakPP) * parseInt(this.state.ValueAnak)) + (parseInt(this.state.hargaInfant) * parseInt(this.state.ValueInfant))});
  }
}
cekJadwal=()=> {
  ...
  this.setState({lengthJadwal:[],show_ringkasan:false,});
    ...
    .then((responseJson) => {
      ...
      if(this.state.lengthJadwal.length>1){
       ...
       if(this.state.jadwalPergi==false){
        this.setState({BottomPopUp:true,show_ringkasan:false, tampilDetailJadwal1:[]});
        }
        if(this.state.jadwalPergi==true){
          this.setState({show_ringkasan:true,tampilJadwal2:true,tampilUbahJadwal:true});
        }
      }
      else{
    ...
    .then((responseJson) => {
     ...
     this.setState({rpHargaDewasa:this.state.lengthJadwal[0].HARGAEXEDEWASA});
      ...
      if(this.state.lengthJadwal.length>1){
       ...
       if(this.state.jadwalPergi==false){
        this.setState({BottomPopUp:true,show_ringkasan:false,});
        }
        if(this.state.jadwalPergi==true){
  ...
  body: JSON.stringify({
    ...
    .then((responseJson) => {
      ...
      if(this.state.KelasTiket=='VIP'){
        ...
        this.setState({tempHargaDewasa:this.state.listPP[0].HARGAVIPDEWASA});
        this.setState({tempHargaAnak:this.state.listPP[0].HARGAVIPANAK});
      ...
      if(this.state.KelasTiket=='Eksekutif'){
        ...
        this.setState({rpHargaAnakPP:this.state.listPP[0].HARGAEXEANAK});
         this.setState({tempHargaDewasa:this.state.listPP[0].HARGAEXEDEWASA});
        this.setState({tempHargaAnak:this.state.listPP[0].HARGAEXEANAK});
      ...
      if(this.state.listPP.length>1){
       ...
       if(this.state.jadwalPulang==false){
        ...
        if(this.state.jadwalPulang==true){
          this.setState({show_ringkasan:true,tampilJadwal2:false,tampilUbahJadwal2:true,tampilUbahJadwal:true, tampilPP:true,});
        }
      }
      else{
...
getKupon=(id_kupon_saya,id_kupon,id_user,status)=>
 ...
 saveData = () =>{let AsyncNomorPesanan  = this.state .NomorPesanan ;
        ...
        let AsyncTotalHarga = this.state.rpTotalHarga;
      ...
      <Dialog
        ...
        <Header style={{backgroundColor:'#3F81B5',}}>
         ...
         <View style={styles.view}>
          <Title style={{marginTop:15,fontFamily:'SourceSansProBold', fontSize:21}}>{this.state.koderute2}Cari Tiket</Title>
          <Right>
              ...
              <View style={{flexDirection:'row', marginTop:15, alignItems:'center'}}>
                <View style={{width:'40%', alignItems:'center'}}>
                  ...
                  <DatePicker
                    ...
                    onDateChange={(date) => {this.setState({date: date,colorbg:'#F4F0E5',viewvisiblePenumpang:true,show_ringkasan:false, jadwalPergi:false }),this.cekJadwal(),this.toggleSwitch()}}
                  />
                  </View>:
                  ...
                  {this.state.viewvisiblePP?<Text style={{fontSize: 15, color:'#000', marginTop:9, fontWeight: 'bold', position:'absolute'}}>{moment(this.state.date2).format('DD MMM YYYY')}</Text>:<Icon name='add' style={{fontSize:30,marginTop:4,position:'absolute'}} />}
                   ...
                   onDateChange={(date) => {this.setState({date2: date, JenisTiket:'PP',tampilJadwal2:false,tampilDetailHarga:false,tampilJadwal:false,tampilClose:true}),this.cekPPTanggal(),this.toggleSwitchPP()}}
                  />
                  <View>
                    {
                  ...
                  <View style={{position:'relative', right:0, height:30, width:30, marginLeft:125, marginTop:-40}} >
                    ...
                    <Icon active name="md-close-circle" style={{fontSize: 22, color:'red'}}/>
                    </TouchableOpacity>
                  </View>
                  :null
                  }
